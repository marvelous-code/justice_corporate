...
class _BatteriesState extends State<Batteries> {
  ...
  void _addItem() async {
    ...
    if (text.isNotEmpty) {
      setState(() {
        _batteryBrands.add(text); // ✅ local update
        _controller.clear();
      });
    ...
    try {
      ...
      if (snapshot.exists) {
        ...
        if (data != null && data["brands"] is List) {
          setState(() {
            _batteryBrands = List<String>.from(data["brands"]);
            selectedOption = _batteryBrands[0];
          });
        }
      }
    } catch (e) {
                                ...
                                if (isClicked)
                                  SizedBox(
                                    ...
                                    child: ListView.builder(
                                      shrinkWrap: true,
                                      itemCount: _batteryBrands.length,
                                      itemBuilder: (context, index) {
                                        final torchBrand =
                                            _batteryBrands[index];
                                        return ListTile(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  ListView.builder(
                                    ...
                                    itemCount: _batteryBrands.length,
                                    itemBuilder: (context, index) {
                                      final torchBrand = _batteryBrands[index];
                                      return ListTile(
                                        title: Text(torchBrand),
                                        leading: Radio<String>(
                                          fillColor: WidgetStateProperty.all(
